...
function NoteTreeItem({ item, children }: { item: OrgItem; children?: React.ReactNode }) {
    ...
    const isDone = item.todoKeyword === 'DONE' || item.todoKeyword === 'CANC';
    const todoColor = isDone ? '#888' : '#f59e0b';
            ...
            >
                ...
                <View style={styles.contentColumn}>
                    <View style={styles.textRow}>
                        ...
                        <RenderLinks
                            title={item.title}
                            style={[styles.title, isDone && styles.done]}
                        />
                    </View>
...
function buildTree(
    ...
    while (index < items.length) {
        ...
        if (item.level === level) {
            ...
            index = newIndex;
            nodes.push(
                <NoteTreeItem key={`${item.sourceUri}-${index}`} item={item}>
...
export function NoteTree({ items }: { items: OrgItem[] }) {
